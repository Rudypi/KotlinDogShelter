...
class DogShelter {
    ...
    fun service(command: Array<String>) {
        if (!this::cages.isInitialized) {
            ...
            when (command[0]) {
                ...
                "spot_by_sex" -> if (isNull(command)) errorBounds() else info(5, command[1])
    ...
    private fun register(color: String, id: String, dogBreed: String, alimentation: String, sex: String) {
        ...
        for (index in cages.indices) {
            if (cages[index]) {
                println("A $color dog named $id is occuping cage ${index + 1}. It's a $dogBreed $sex and feeds on $alimentation")
                cages[index] = false
            ...
            if(!breed.contains(dogBreed)) breed.also { println("Dog breed not referenced, adding it to the list") }.add(dogBreed)
        }
    ...
    private fun info(find: Int, search: String) {
        ...
        if (results is String && results.isNullOrEmpty()) {
            println(
                    when (find) {
                        0 -> "Cage is empty."
                        ...
                        else -> "No $search dogs were found."
                    }
            )
        } else if (results !is String) throw Exception("Error in command")
          else println(
    ...
    private fun help() {
        println(
            ...
            spot_by_sex - returns the dog names based on their sex
